...
import { ChevronRight, MessageCircle, Copy, Check } from 'lucide-react';
import { papers, SUBJECT_EMOJI } from '../../../data/papers';
import PaperCard from '../../../components/PaperCard';
import RectangleAd from '../../../components/ads/RectangleAd';

export default function PaperViewClient({ slug }: { slug: string }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <Link href="/browse" className="bg-green-600 hover:bg-green-700 text-white font-medium px-6 py-2.5 rounded-full transition-colors">Browse Papers</Link>
        </div>
      </div>
    );
  }
  ...
  function handleView() {
    if (paper) window.open(paper.viewUrl, '_blank');
  }
  ...
  const whatsappText = `Free ${paper.subject} past papers for ${paper.class}: ${typeof window !== 'undefined' ? window.location.href : ''}`;
        ...
        {related.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href={`/browse?class=${encodeURIComponent(paper.class)}`} className="text-sm text-green-600 hover:text-green-700 font-medium flex items-center gap-1">
